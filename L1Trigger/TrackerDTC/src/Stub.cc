...
namespace trackerDTC {
  ...
  Stub::Stub(const trackerTFP::DataFormats* dataFormats, const tt::SensorModule* sm, const TTStubRef& ttStubRef)
    ...
    if (ttStubRef->clusterRef(1).isNull()) {
      bend_ = 0; // Default bend for single-cluster stubs (corresponds to infinite pt)
    } else {
      bend_ = std::floor(std::pow(-1, sm_->signBend()) * (ttStubRef->bendBE()) / setup_->baseBend());
    }
    ...
    // Check if bend_ fits in the hardware bit width
    const tt::SensorModule::Type type = sm_->type();
    const int bendWidth = setup_->hybridWidthBend(type);
    const int maxBend = (1 << (bendWidth - 1)) - 1;
    const int minBend = -(1 << (bendWidth - 1));
    if (bend_ > maxBend || bend_ < minBend) {
      valid_ = false;
    }
  ...
  Stub::Stub(const tt::Setup* setup,
    ...
    // Check if the encoded bend_ fits in the hardware bit width
    const int bendWidth = setup->hybridWidthBend(sm->type());
    const int maxBend = (1 << (bendWidth - 1)) - 1;
    const int minBend = -(1 << (bendWidth - 1));
    if (bend_ > maxBend || bend_ < minBend) {
      valid_ = false;
