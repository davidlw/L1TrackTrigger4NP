...
/*  if (std::abs(t) < 0.1) {
    phiproj = phi0;
    rproj = 1e6; // Force the radius match to fail safely
    phider = 0.0;
    rder = 0.0;
    return;
  }
...
bool TrackletCalculatorBase::barrelSeeding(const Stub* innerFPGAStub,
  ...
  for (unsigned int i = 0; i < N_LAYER - 2; ++i) {
    ...
    if (settings_.projlayers(iSeed_, i) <= 3) {  //TODO clean up logic
      ...
      // Clamp so value does not hit atExtreme after down-shift (mirrors TrackletCalculatorDisplaced)
      if (iphiproj[i] >= (1 << (int)settings_.nphibitsstub(settings_.projlayers(iSeed_, i) - 1)) - 1)
        iphiproj[i] = (1 << (int)settings_.nphibitsstub(settings_.projlayers(iSeed_, i) - 1)) - 2;
    } else {
...
bool TrackletCalculatorBase::diskSeeding(const Stub* innerFPGAStub,
  ...
  for (unsigned int i = 0; i < N_DISK - 2; ++i) {
    ...
    // Clamp so value does not hit atExtreme after down-shift (mirrors TrackletCalculatorDisplaced)
    if (iphiproj[i] >= (1 << (int)settings_.nphibitsstub(0)) - 1)
      iphiproj[i] = (1 << (int)settings_.nphibitsstub(0)) - 2;
