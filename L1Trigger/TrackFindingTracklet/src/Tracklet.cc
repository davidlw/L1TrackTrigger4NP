...
void Tracklet::setFitPars(double rinvfit,
  ...
  if (irinvfit > (1 << (settings_.nbitsrinvfit() - 1)))
    irinvfit = (1 << (settings_.nbitsrinvfit() - 1));
  if (irinvfit <= -(1 << (settings_.nbitsrinvfit() - 1)))
    irinvfit = -(1 << (settings_.nbitsrinvfit() - 1)) + 1;
  fpgafitpars_.rinv().set(irinvfit, settings_.nbitsrinvfit(), false, __LINE__, __FILE__);
  fpgafitpars_.phi0().set(iphi0fit, settings_.nbitsphi0fit(), false, __LINE__, __FILE__);
  fpgafitpars_.d0().set(id0fit, settings_.nbitsd0fit(), false, __LINE__, __FILE__);
  fpgafitpars_.t().set(itfit, settings_.nbitstfit(), false, __LINE__, __FILE__);
